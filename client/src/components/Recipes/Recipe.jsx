...
const Recipe = ({ recipe }) => {
  ...
  const { mutate: deleteRecipe } = useMutation({
    ...
    onSuccess: () => {
      ...
      // מחדש את כל הקוויריז הקשורים למתכונים
  ...
  return (
    <div className="max-w-sm mx-auto p-4 bg-white shadow-md rounded-lg flex flex-col items-center">
      {/* Recipe Image */}
      <div 
        className="mb-4 w-full h-64 bg-gray-200 rounded-md flex items-center justify-center overflow-hidden cursor-pointer hover:opacity-90 transition-opacity" 
      ...
      {/* Recipe Type */}
      <p className="w-fit text-white px-3 py-1 rounded-lg bg-purple-700 text-sm mb-2">
        Recipe Type: {linkedHousehold ? 'Public' : 'Private'}
      </p>
      
      <button 
        onClick={() => deleteRecipe()} 
        className="w-full cursor-pointer text-white px-3 py-1 rounded-lg bg-rose-700 text-sm hover:bg-rose-800 transition-colors"
      >
        Delete
      </button>
    </div>
